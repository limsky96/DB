...
-- 총 사원수
SELECT COUNT(*) FROM EMP; -- 가장 많이 사용함 (그룹함수의 대표적)
-- 중복 제거를 반영하여 담당업무의 개수를 구하는 쿼리문 
SELECT COUNT(DISTINCT JOB) as 업무수 FROM EMP; -- as 생략가능 
...
-- 1. 컬럼은 group by 한 컬럼 명이 올수있다.
-- 2. 집합 함수가 올 수 있다. 단일 함수는 올 수가 없다.
...
SELECT DEPTNO, AVG(SAL) FROM EMP GROUP BY DEPTNO HAVING AVG(SAL) >= 2000; -- having을 사용 